...
This is a hand-built P0 snapshot, not generated. P9 (`docs/tenant_isolation_and_platform_admin_plan.md`, P9 task 9) replaces the *generation* of a file like this with a script that reads `apps/api/prisma/schema.prisma`, every controller's route decorators, `apps/api/src/modules/ws/screen.gateway.ts`, and every storage-key call site, then fails CI when the output differs from what's checked in or contains an unclassified item. This file's structure — four sections, each a flat three-column table — is intentionally chosen to make that future diff mechanical. Until P9 lands, update this file by hand per the PR template checklist in `.github/PULL_REQUEST_TEMPLATE.md` whenever a model, route, event, or storage prefix is added or changed.
