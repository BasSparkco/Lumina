...
import { Plus, PenTool, Globe, Users, EyeOff, Archive, } from 'lucide-react';
    ...
    >
      ...
      <label className="text-xs font-medium text-[var(--deck-text-mid)]">
        ...
        <select className={`${inputClass} mt-1`} value={visibility} onChange={(e) => setVisibility(e.target.value as TemplateVisibility)}>
          {VISIBILITIES.map((v) => (
            <option key={v} value={v}>{tr(`visibility.${v}`)}</option>
          ))}
        </select>
      </label>
...
function TenantAccessModal({ template, onClose }: { template: DesignTemplate; onClose: () => void }) {
  ...
  return (
    <Modal title={tr('accessTitle', { name: template.name })} onClose={onClose}>
      ...
      <div className="max-h-64 space-y-1 overflow-y-auto rounded-lg border border-[var(--deck-glass-border)] p-2">
        {orgs.map((org) => (
          <label key={org.id} className="flex items-center gap-2 rounded px-1.5 py-1 text-sm hover:bg-[var(--deck-glass-fill-strong)]">
            ...
            <span className="ms-auto text-[11px] text-[var(--deck-text-low)]">{org.slug}</span>
          </label>
        ))}
      </div>
...
export default function AdminTemplatesPage() {
  ...
  return (
    <div className="signal-page signal-workspace-page signal-admin-templates-page space-y-6">
      <div className="signal-page-heading">
        <div>
          <h1 className="text-2xl font-bold text-[var(--deck-text-hi)]">{tr('title')}</h1>
        ...
        >
          <Plus className="h-4 w-4" /> {tr('newTemplate')}
        </button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-[var(--deck-glass-border)]">
        <table className="w-full text-sm">
          <thead className="bg-[var(--deck-glass-fill-strong)] text-start text-xs uppercase tracking-wide text-[var(--deck-text-mid)]">
            <tr>
              <th className="px-4 py-2">{tr('name')}</th>
              <th className="px-4 py-2">{tr('category')}</th>
              <th className="px-4 py-2">{tr('statusLabel')}</th>
              <th className="px-4 py-2">{tr('visibilityLabel')}</th>
              <th className="px-4 py-2">{tr('version')}</th>
              <th className="px-4 py-2">{tr('clones')}</th>
              <th className="px-4 py-2" />
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--deck-glass-border-soft)]">
            {isLoading && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-[var(--deck-text-low)]">{tr('loading')}</td></tr>
            )}
            {!isLoading && templates.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-[var(--deck-text-low)]">{tr('empty')}</td></tr>
            )}
            {templates.map((t) => (
              <tr key={t.id} className="text-[var(--deck-text-hi)]">
                <td className="px-4 py-2 font-medium">{t.name}</td>
                <td className="px-4 py-2 text-xs text-[var(--deck-text-mid)]">{tr(`categories.${t.category}`)}</td>
                <td className="px-4 py-2">
                  <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${STATUS_STYLES[t.status]}`}>{tr(`status.${t.status}`)}</span>
                </td>
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {t.status !== 'ARCHIVED' && (
                      <button
                        title={tr('archive')}
                        onClick={() => {
                          if (confirmDelete(tr('archiveConfirm', { name: t.name }))) archiveMut.mutate(t.id);
                        }}
      ...
      {showCreate && (
        <Modal title={tr('newTemplate')} onClose={() => setShowCreate(false)}>
          <MetadataForm
      ...
      {editing && (
        <Modal title={tr('editTitle', { name: editing.name })} onClose={() => setEditing(null)}>
          <MetadataForm
