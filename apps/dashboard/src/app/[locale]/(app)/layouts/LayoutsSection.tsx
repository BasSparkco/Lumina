...
export function LayoutsSection({ onSelectTab }: { onSelectTab: (tab: 'layouts' | 'themes') => void }) {
  ...
  function zoneContextMenuActions(i: number, key: string, z: ZoneInput): ContextMenuAction[] {
    ...
    return [
      ...
      ...(asset?.type === 'IMAGE'
        ? [{ key: 'hideBg', label: tc('hideBackground'), icon: Wand2, onClick: () => void hideZoneBackground(i, key) }]
        : []),
      {
        key: 'delete',
        label: tc('delete'),
                ...
                <div
                  ...
                  onContextMenu={(e) => {
                    ...
                    setContextMenu({
                      x: e.clientX,
                      y: e.clientY,
                      actions: [{ key: 'add', label: t('addZone'), icon: Plus, onClick: () => setAddPanelOpen(true) }],
                    });
                  }}
