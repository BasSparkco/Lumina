...
async function req<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (res.status === 401) { clearToken(); if (typeof window !== 'undefined') window.location.replace('/en/login'); }
  if (!res.ok) {
...
export const screensApi = {
  ...
  updatePrayer: (id: string, data: { latitude?: number; longitude?: number; prayerMethod?: string; athanEnabled?: boolean }) =>
...
export interface User { id: string; email: string; name: string; role: string; orgId: string; }
...
export interface ScheduleEntry extends CreateScheduleInput {
  id: string; createdAt: string; playlist: { id: string; name: string };
}
export interface Screen {
  ...
  prayerMethod: string; athanEnabled: boolean;
}
