    ...
    case 'PRAYER':
      return (
        <div className="grid grid-cols-3 gap-2 rounded bg-amber-50 p-2 text-xs dark:bg-amber-950/40">
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('methodOverride')}</label>
            <select
              value={(cfg.method as string) ?? ''}
              onChange={(e) => onChangeCommitted({ ...cfg, method: e.target.value || undefined })}
              className="w-full rounded border border-[var(--deck-glass-border)] px-1.5 py-1 focus:outline-none"
            >
          ...
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('language')}</label>
            <select
          ...
          <label className="flex cursor-pointer items-center gap-1.5 self-end">
            ...
            />
            <span className="text-[var(--deck-text-mid)]">{t('athanAudio')}</span>
          </label>
        </div>
      );
    case 'WEATHER':
      return (
        <div className="grid grid-cols-2 gap-2 rounded bg-sky-50 p-2 text-xs dark:bg-sky-950/40">
          ...
          <p className="self-center text-[var(--deck-text-low)]">{t('locationInherited')}</p>
        </div>
      );
    case 'CURRENCY':
      return (
        <div className="grid grid-cols-2 gap-2 rounded bg-emerald-50 p-2 text-xs dark:bg-emerald-950/40">
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('baseCurrency')}</label>
            <select
      ...
      return (
        <div className="space-y-2 rounded bg-orange-50 p-2 text-xs dark:bg-orange-950/40">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('tickerSource')}</label>
              <select
                value={sourceType}
            ...
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('scrollDirection')}</label>
              <select
          ...
          {sourceType === 'text' ? (
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('tickerText')}</label>
              <textarea
                ...
                className="w-full resize-none rounded border border-[var(--deck-glass-border)] px-2 py-1 focus:ring-1 focus:ring-orange-400 focus:outline-none"
              />
            </div>
          ) : (
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('rssFeedUrl')}</label>
              <input
                type="url"
                ...
                className="w-full rounded border border-[var(--deck-glass-border)] px-2 py-1 focus:ring-1 focus:ring-orange-400 focus:outline-none"
              />
            </div>
          )}
        </div>
    ...
    case 'TIME':
      return (
        <div className="grid grid-cols-2 gap-2 rounded bg-violet-50 p-2 text-xs dark:bg-violet-950/40">
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('clockFormat')}</label>
            <select
          ...
          <label className="flex cursor-pointer items-center gap-1.5 self-end">
            ...
            />
            <span className="text-[var(--deck-text-mid)]">{t('showSeconds')}</span>
          </label>
          <div className="col-span-2">
            <label className="mb-0.5 flex cursor-pointer items-center gap-1.5">
              ...
              />
              <span className="text-[var(--deck-text-mid)]">{t('timezoneOverride')}</span>
            </label>
            {cfg.timezone != null && (
              <TimezoneSelect
    ...
    case 'DATE':
      return (
        <div className="grid grid-cols-2 gap-2 rounded bg-teal-50 p-2 text-xs dark:bg-teal-950/40">
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('dateFormat')}</label>
            <select
    ...
    case 'QR':
      return (
        <div className="space-y-2 rounded bg-rose-50 p-2 text-xs dark:bg-rose-950/40">
          <div>
            <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('qrValue')}</label>
            <input
              ...
              className="w-full rounded border border-[var(--deck-glass-border)] px-2 py-1 focus:ring-1 focus:ring-rose-400 focus:outline-none"
            />
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('qrForeground')}</label>
              <input
                type="color"
                value={(cfg.color as string) ?? '#000000'}
                onChange={(e) => onChangeCommitted({ ...cfg, color: e.target.value })}
                className="h-7 w-full cursor-pointer rounded border border-[var(--deck-glass-border)] p-0.5"
              />
            </div>
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('qrBackground')}</label>
              <input
                type="color"
            ...
            <div>
              <label className="mb-0.5 block text-[var(--deck-text-mid)]">{t('qrSize')}</label>
              <input
                type="number"
                min={20}
                max={100}
