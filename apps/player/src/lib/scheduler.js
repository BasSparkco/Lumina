...
export function resolveSchedule(rules, now) {
    const day = now.getDay();
    const hhmm = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
    const matching = rules.filter(r => {
        if (r.startDate && now < new Date(r.startDate))
            return false;
        if (r.endDate && now > new Date(r.endDate))
            return false;
        if (r.daysOfWeek.length > 0 && !r.daysOfWeek.includes(day))
            return false;
        if (r.startTime && r.endTime) {
            if (r.startTime <= r.endTime) {
                if (hhmm < r.startTime || hhmm >= r.endTime)
                    return false;
            }
            else {
...
export function msUntilNextTransition(rules, now) {
