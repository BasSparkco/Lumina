...
/** Incremental SHA-256 so a multi-gigabyte OPFS file never enters JS memory as one buffer. */
export class IncrementalSha256 {
  ...
  private compress() {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h! + sum1 + choice + ROUND_CONSTANTS[index]! + this.words[index]!) >>> 0;
